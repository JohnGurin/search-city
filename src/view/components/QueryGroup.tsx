import { QueryButton } from './QueryButton'
import { QueryInput } from './QueryInput'


export const QueryGroup = () => (
  <div className="relative">
    <div className={String.raw`:uno:
      absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none
    `}
    >
      <svg
        className=":uno: w-4 h-4 text-gray-500 dark:text-gray-400"
        aria-hidden="true"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 20 20"
      >
        <path
          stroke="currentColor"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth="2"
          d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"
        />
      </svg>
    </div>
    <QueryInput
      type="search"
      name="query"
      className={String.raw`:uno:
        block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300
        rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500
        dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400
        dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500
      `}
      placeholder="Search city..."
    />
    <QueryButton
      type="button"
      className={String.raw`:uno:
        text-white absolute end-2 bottom-2 top-2 bg-blue-700 w-80px
        hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300
        rounded-lg line-height-none px-4 py-2
        dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800
      `}
    />
  </div>
)
