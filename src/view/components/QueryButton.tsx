import { state, present } from '#/init'
import { match as indicator_match } from '#/model/indicator'
import type { ButtonHTMLAttributes, ReactNode } from 'react'
import { useSubscribe } from '#/view/useSubscribe'


export const QueryButton = (props: ButtonHTMLAttributes<HTMLButtonElement>) => {
  const indicator = useSubscribe(state.fetch_indicator)
  const label = indicator_button_label(indicator)
  const disabled = indicator_button_disabled(indicator)
  return (
    <button
      {...props}
      type="button"
...
const SPINNER = (
  <div role="status">
    <svg
      aria-hidden="true"
      className=":uno: inline w-4 h-4 text-gray-200 animate-spin fill-blue-600"
      viewBox="0 0 100 101"
      fill="none"
...
const BUTTON_TITLE = 'query'

const indicator_button_label = indicator_match<string, ReactNode>({
  idle: BUTTON_TITLE,
  delaying: '...',
  loading: SPINNER,
  error: BUTTON_TITLE,
...
const indicator_button_disabled = indicator_match<string, boolean>({
  idle: false,
  delaying: true,
  loading: true,
  error: false,
